import { tool } from "ai";
import { z } from "zod";
import { addRepoDeployment, readRepoMetadata } from "./repo-storage-local";
import { WORKDIR, VM_PORT } from "../vars";
import { type Vm } from "./docker-vm";

const DEPLOYMENT_DOMAIN_SUFFIX = "adorable.style.dev";
const getDomainForCommit = (commitSha: string) =>
  `${commitSha.slice(0, 12)}-${DEPLOYMENT_DOMAIN_SUFFIX}`;
  ...
  const commitTool = tool({
    ...
    execute: async ({ message }) => {
      const gitCommand = `git -C ${shellQuote(WORKDIR)} config user.name ${shellQuote(
        ...
        "forge@kanam.local",
      )} && git -C ${shellQuote(WORKDIR)} add -A && git -C ${shellQuote(
        WORKDIR,
      ...
      if (commitResult.ok && options?.metadataRepoId) {
        void (async () => {
          ...
          // Local MVP: no serverless deploy. Record the deployment as "idle"
          // (no live domain) so the UI timeline shows the commit.
